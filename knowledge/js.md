...
一张图搞定（https://github.com/mqyqingfeng/Blog/issues/2）
...
基本类型储存在栈内存中，但是一旦被闭包引用则成为常驻内存，会储存在堆内存中
复杂类型会储存在堆内存中
...
V8 将堆内存分成新生代空间和老生代空间。

1. 新生代空间: 用于存活较短的对象
   （1）又分成两个空间: from 空间 与 to 空间
   （2）Scavenge GC 算法: 当 from 空间被占满时，启动 GC 算法
...
1. 老生代空间: 用于存活时间较长的对象
   ...
   1.1 从 新生代空间 转移到 老生代空间 的条件

   - 经历过一次以上 Scavenge GC 的对象
   - 当 to space 体积超过 25%

     1.2 标记清除算法: 标记存活的对象，未被标记的则被释放

   - 增量标记: 小模块标记，在代码执行间隙执，GC 会影响性能
   - 并发标记(最新技术): 不阻塞 js 执行

     1.3 压缩算法: 将内存中清除后导致的碎片化对象往内存堆的一端移动，解决 内存的碎片化
...
1. 意外的全局变量，无法被回收
2. 闭包，未被正确关闭，导致所引用的外部变量未被释放
3. 定时器，没有正确的被销毁（低版本浏览器可能出现）
...
JS 在浏览器中运行时采用的时单线程模式，
在服务端 nodejs 中运行时采用的是单进程模式
